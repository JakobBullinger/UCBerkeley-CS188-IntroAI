...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variables = [PAC, GHOST0, OBS0, GHOST1, OBS1]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # Join and eliminate by variable
        currentFactorsList = bn.BayesNet.getAllCPTsWithEvidence(bayesNet, evidenceDict)
        for eliminationVariable in eliminationOrder:
            ...
            # Determine if elimination is necessary
            if len(joinedFactor.unconditionedVariables()) != 1:
                joinedFactors = eliminate(joinedFactor, eliminationVariable)
                currentFactorsList.append(joinedFactors)

        # Create and normalize final factor
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # If the ghost is in jail and the noisy distance is None, then the probability is 1
        ...
        # If the ghost is not in jail and the noisy distance is None, then the probability is 0
        elif noisyDistance == None:
            return 0
            
        # If the ghost is not in jail, then the noisy distance is the manhattan distance
        else:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            newProbability = self.getObservationProb(observation, pacmanPosition, position, jailPosition)
            previousProbability = self.beliefs[position]
            self.beliefs[position] = previousProbability * newProbability
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Define empty dictionary for updated beliefs
        ...
        for oldPosition in self.allPositions:
            ...
            # Iterate over all possible new positions
            newPositionDistance = self.getPositionDistribution(gameState, oldPosition)
            for newPosition, transitionProbability in newPositionDistance.items():
                if newPosition in updatedBeliefs:
                    updatedBeliefs[newPosition] += oldBelief * transitionProbability
                else:
                    updatedBeliefs[newPosition] = oldBelief * transitionProbability
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for index in range(self.numParticles):
            positionIndex = index % len(self.legalPositions)
            position = self.legalPositions[positionIndex]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Define a dictionary for initial beliefs
        weights = DiscreteDistribution()

        # Iterate over all particles
        for particle in self.particles:
            newWeight = self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
            oldWeight = weights[particle]
            weights[particle] = oldWeight + newWeight
    ...
    def elapseTime(self, gameState):
        ...
        newParticles = []
        for particle in self.particles:
            newParticle = self.getPositionDistribution(gameState, particle).sample()
            newParticles.append(newParticle)
        self.particles = newParticles
        "*** END YOUR CODE HERE ***"
